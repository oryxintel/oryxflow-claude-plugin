---
description: Bring an existing d6tflow project's scaffold floor up to date with the latest bundled template - diff the floor files (CLAUDE.md, .gitignore, viz-template.ipynb, docs skeleton) against the current template, propose a migration plan, and apply it only after the user confirms. Never touches the user's pipeline files.
disable-model-invocation: true
---

# Update a d6tflow project to the latest scaffold

A project scaffolded by `/d6tflow:init-project` a while ago can drift from the
...
It is the maintenance counterpart to `/d6tflow:init-project` (which bootstraps and
...
## 1. Load the d6tflow skill first

The plugin's `d6tflow` skill is what lets you tell scaffold FLOOR (plugin-owned,
...
- **FLOOR (reconcile by default).** Files the plugin owns AND that actually
  ...
  - `CLAUDE.md` - the portable floor and the usual reason to run this; it carries
    the conventions AND the `<!-- d6tflow-floor: VERSION -->` stamp.
...
- **LOW-CHURN (additive / on demand only).** Floor files that almost never change
  ...
  - `.gitignore` - rarely changes, and `/d6tflow:init-gitlfs` deliberately edits
...
- **SKELETON (reconcile structure only).** Files that ship as a placeholder and
  ...
  - `docs/d6tflow-data.md` (ships with a `PLACEHOLDER` line; if the project has
...
The `CLAUDE.md` floor stamp (`<!-- d6tflow-floor: VERSION -->`) is what the skill
...
- What the template ADDS that the project lacks (e.g. the "check the `d6tflow`
