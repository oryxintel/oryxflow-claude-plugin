# Plan: plugin-first documentation model + `/oryxflow:project-init` scaffold

## Context

This repo is the source of the `oryxflow` Claude Code plugin. Through discussion we
settled a plugin-first architecture for how a oryxflow data-science flow gets
documented and how Claude works with it. The change implements that model:

- **Generic oryxflow knowledge** lives only in the plugin (SKILL + reference +
...
- A new slash command **`/oryxflow:project-init`** scaffolds a new project by
...
- Vendoring: **unpacked mirror** at `resources/template-minimal/` (already staged
  by the user). Source of truth is the separate `oryxflow-template-minimal` repo;
...
- Command namespacing (confirmed): `commands/project-init.md` -> `/oryxflow:project-init`.
...
- ML patterns: **separate on-demand file** `skills/oryxflow/ml-patterns.md`.
- Docs: harvest `claude-oryxflow-ml.md` -> `ml-patterns.md`; delete the other three.
...
- Pre-flight: if `${CLAUDE_PROJECT_DIR}` already looks like a oryxflow project
...
- `VENDORED.md` - "Mirror of oryxflow-template-minimal (<url>) @ <SHA>. Do not edit
...
1. One line: this is a oryxflow data-science project.
...
4. Compact file map.
5. Pointer: if the oryxflow plugin is installed, its skill covers this in depth.
...
### 3. Skill rework (`skills/oryxflow/SKILL.md`)
...
### 4. ML patterns (plugin: new `skills/oryxflow/ml-patterns.md`)
Harvest from `docs/claude-oryxflow-ml.md`: ML pipeline architecture; the 5 core
...
- Delete `docs/claude-oryxflow-ml.md` AFTER harvesting into `ml-patterns.md`.
...
- `README.md`: add `commands/`, `resources/template-minimal/`, and
  `skills/oryxflow/ml-patterns.md` to the Contents tree; add a "Quickstart: start a
  new project" section documenting `/oryxflow:project-init`.
...
## Critical files
- New: `commands/project-init.md`, `skills/oryxflow/ml-patterns.md`,
...
- Edit: `skills/oryxflow/SKILL.md`, `docs/design-notes.md`, `README.md`,
  `.claude-plugin/plugin.json`, `docs/CHANGELOG.md`.
- Delete: `docs/claude-project.md`, `docs/claude-data-doc.md`,
  `docs/claude-ml-plan.md`, `docs/claude-oryxflow-ml.md`.
...
5. Plain `/oryxflow:oryxflow` in the scaffolded dir: confirm it reads the docs, sees
